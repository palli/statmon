...
//  Constants
 ...
 var cZoomBoxTopOffsetWText = 31 - 1;		// With text (was: 17)

 // Distance from right edge of image to right edge of graph
 var cZoomBoxRightOffset = -27;

 var cZoomSensitiveZoneName = "zoomSensitiveZone";
 var cZoomSensitiveZoneOffset = 10;
...
// var imgSource;
 ...
 zoomBoxWidth = parseInt(gUrlObj.getUrlParameterValue("imagewidth")) + 1;
 zoomBoxHeight = parseInt(gUrlObj.getUrlParameterValue("imageheight")) + 1;
...
function initEvents() {
 ...
 if (gBrowserObj.browser == "Netscape") {
  document.addEventListener(Event.MOUSEMOVE, document.onmouseup, false);
  document.addEventListener(Event.MOUSEDOWN, document.onmouseup, false);
  document.addEventListener(Event.MOUSEUP, document.onmouseup, false);
 }
}
...
function onMouseUpEvent(e) {
 ...
 graphStart = parseInt(gUrlObj.getUrlParameterValue("graphstart"));
 graphEnd = parseInt(gUrlObj.getUrlParameterValue("graphend"));
 idxStr = "";
 countStr = "";
 ...
 if ((gMouseObj.rightButtonPressed()) && (insideZoomBox())) {
  ...
  var graphWidth = gUrlObj.getUrlParameterValue("imagewidth");
  var graphHeight = gUrlObj.getUrlParameterValue("imageheight");
  ...
  //open(urlBase + "&page=graph&&host=" + host + "&service=" + service + "&disp=" + dispName + idxStr + countStr + "&graph_start=" + newGraphStart + "&graph_end=" + newGraphEnd + "&graph_height=" + graphHeight + "&graph_width=" + graphWidth, "_self");
  open(urlBase + "&" + myArguments + "&graphstart=" + newGraphStart + "&graphend=" + newGraphEnd, "_self")
 }
 ...
 if ((gMouseObj.leftButtonPressed()) && (gMouseObj.dragging)) {
  ...
  if ((minX != maxX) || (minY != maxY)) {
   ...
   //open(urlBase + "&key=" + key + "&page=graph&host=" + host + "&service=" + service + "&disp=" + dispName + idxStr + countStr + "&graph_start=" + newGraphStart + "&graph_end=" + newGraphEnd + "&graph_height=" + graphHeight + "&graph_width=" + graphWidth, "_self");
   //open(urlBase + myArguments + "&graph_start=" + newGraphStart + "&graph_end=" + newGraphEnd, + "&graph_height=" + graphHeight + "&graph_width=" + graphWidth, "_self");
   open(urlBase + "&" + myArguments + "&graphstart=" + newGraphStart + "&graphend=" + newGraphEnd, "_self")
  }
 }
}
